...
function start() {
  // Hide the old audio controls
  const oldAudioControls = document.getElementById('audio-controls');
  if (oldAudioControls) {
    oldAudioControls.style.display = 'none';
  }
  ...
  document.getElementById('desktop-tutorial-overlay').addEventListener('click', async () => {
    if (THREE.AudioContext) {
      THREE.AudioContext.getContext().resume();
    }
  });

  document.getElementById('close-desktop-tutorial').addEventListener('click', async () => {
    if (THREE.AudioContext) {
      THREE.AudioContext.getContext().resume();
    }
  });
...
function setupSomaFMAudio() {
  ...
  // Add click event listener directly to the play button
  ...
  volumeLabel.textContent = 'Volume: 70%';
  volumeLabel.style.color = 'white';
  volumeLabel.style.fontSize = '14px';
  volumeLabel.style.marginRight = '5px';
  volumeLabel.id = 'volume-label';

  // Add volume slider to container and add direct event listener
  audioControlsContainer.appendChild(volumeSlider);
  
  // Add direct event listener to volume slider
  volumeSlider.addEventListener('input', function() {
    const volume = parseFloat(this.value);
    console.log("Volume changed directly to:", volume);
    
    // Update audio element volume
    if (audioElement) {
      audioElement.volume = volume;
      console.log("Audio element volume set directly to:", audioElement.volume);
    }
  ...
  // Add credits for SOMA FM
  const creditsDiv = document.createElement('div');
  creditsDiv.id = 'credits';
  creditsDiv.style.color = 'white';
  creditsDiv.style.fontSize = '12px';
  creditsDiv.style.marginLeft = '10px';
  creditsDiv.innerHTML = 'Music: <a href="https://somafm.com/" target="_blank" style="color: #4a9df8; text-decoration: none;">SOMA FM</a> | CC BY 4.0';
  audioControlsContainer.appendChild(creditsDiv);
...
function initializeAudioContext() {
  ...
  try {
    ...
    // Only connect the audio element if it's not already connected
    if (!sound) {
      ...
      try {
        ...
        // If we can't connect the audio element, we can still play audio
        // but we won't have visualization
      }
    }
    ...
    // We already added the event listener in setupSomaFMAudio, so we don't need to add it again here
...
function toggleAudio() {
  ...
  if (!isPlaying) {
    ...
    const existingIndicator = playButton.querySelector('span');
    if (existingIndicator) {
      playButton.removeChild(existingIndicator);
    }
    ...
    audioElement.play().then(() => {
      ...
      // Set button text to "Pause" - use innerHTML and make sure it's the only content
      playButton.innerHTML = '';
      const textNode = document.createTextNode('Pause');
      ...
      // Force the button text to be "Pause" after a delay
      setTimeout(() => {
        if (isPlaying) {
          playButton.innerHTML = '';
      ...
      // Add event listener for when audio is actually playing
      const onPlaying = () => {
        // Remove any existing loading indicators again
        const indicator = playButton.querySelector('span');
        if (indicator) {
          playButton.removeChild(indicator);
        }
        
        // Make sure the button text is "Pause"
        playButton.innerHTML = '';
        playButton.appendChild(document.createTextNode('Pause'));
      };
      ...
      playButton.innerHTML = '';
      playButton.appendChild(document.createTextNode('Play Music'));
      playButton.disabled = false;
...
async function setupScene() {
  ...
  camera.position.set(30, playerHeight, 0); // Start at a specific position instead of 0,0,0
  
  // Set initial camera direction
  euler.set(0, Math.PI / 2, 0, "YXZ"); // Look along -Z axis
  ...
  const bloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 0.1, 0.1, 0.25);
  ...
  // OrbitControls - disabled to prevent interference with player movement
  controls = new OrbitControls(camera, renderer.domElement);
  controls.enabled = false; // Disable OrbitControls to allow player movement
...
function addParticles() {
  ...
  // Position particles closer to the player's starting position
  for (let i = 0; i < particleCount; i++) {
    // Center particles around player starting position (30, playerHeight, 0)
    const x = 30 + THREE.MathUtils.randFloatSpread(100);
    const y = playerHeight + THREE.MathUtils.randFloat(0, 50);
  ...
  const texturePath = '/assets/images/dustball.png';
  console.log("Loading texture from:", texturePath);
  
  textureLoader.load(texturePath, (glowTexture) => {
    ...
    const material = new THREE.PointsMaterial({
      map: glowTexture,
      color: 0xffffff,
      size: 3.0, // Much larger size to make particles more visible
      transparent: true,
      alphaTest: 0.1,
      depthWrite: false,
    ...
    // Position the particle system near the player
    particleSystem.position.set(0, 0, 0);
    scene.add(particleSystem);
    console.log("✨ Glowing square particles initialized and added to scene!");
    
    // Log the particle system's position and properties for debugging
    console.log("Particle system position:", particleSystem.position);
    console.log("Particle system material:", material);
    console.log("Particle count:", particleCount);
  },
  (xhr) => {
    console.log(`Particle texture loading: ${Math.round((xhr.loaded / xhr.total) * 100)}%`);
  },
  (err) => {
    console.error("🚨 Error loading texture:", texturePath, err);
    
    // Create particles even without texture as fallback
    const material = new THREE.PointsMaterial({
    ...
    particleSystem = new THREE.Points(geometry, material);
    scene.add(particleSystem);
    console.log("✨ Fallback particles created without texture!");
  });
}
...
function setupPlayer() {
  ...
  player.position.set(30, 0, 0); // Start at a specific position instead of 0,0,0
...
function onMouseMove(event) {
  ...
  if (mouseEnabled) {
    ...
    // Update the euler angles based on mouse movement
    ...
    // Update the player direction based on camera quaternion
...
function animate(time) {
  ...
  if (particleSystem && analyser) {
    ...
    particleSystem.material.size = 1 + pulse * 2.5;
